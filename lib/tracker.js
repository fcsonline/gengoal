...
Tracker.prototype.forceOrderClose = function (order_id) {
  ...
  return Order(this.bookshelf)
    ...
    .then(function (order) {
      ...
      logger.gengo('Forced a branch close for "' + chalk.magenta(branch) + '"');

      completed = order.set('status', 'completed').save();
      repository = self.find(order.get('repository'));
      commit = self.persistAndCloseBranch(order, repository, branch);
 ...
 * @param {String} branch
 */
Tracker.prototype.persistAndCloseBranch = function (order, repository, branch) {
  var self = this;
  ...
  return Order(this.bookshelf).where({status: 'pending'}).fetchAll().then(function(orders){
    ...
    orders.forEach(function(order) {
      ...
      if (order.get('expires_at') < Date.now()) {
        ...
        pull = self.persistAndCloseBranch(order, repository, branch).then(function () {
