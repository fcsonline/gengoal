...
/**
 * Force a branch close
 *
 * @param {String} order_id
 */
Tracker.prototype.forceOrderClose = function (order_id) {
  var self = this;

  return Order(this.bookshelf)
    .where({id: order_id})
    .fetch()
    .then(function (order) {
      var completed
      ...
      logger.gengo('Forced a order close for "' + chalk.magenta(order_id) + '"');

      completed = order.set('status', 'completed').save();
      repository = self.find(order.get('repository'));
      commit = self.persistAndCloseBranch(order, repository);
 ...
 * @param {Repository} repository
 */
Tracker.prototype.persistAndCloseBranch = function (order, repository) {
  var self = this
    , branch;
  ...
  return repository.checkout(branch)
    .then(function () {
      return Copy(self.bookshelf)
        ...
        .then(function (copies) {
          ...
          copies.forEach(function(copy) {
            ...
            key_path = copy.get('key').split('.');
            locale = key_path[0];
            language = repository.find(locale);
            language.set(key_path, copy.get('result'));
      ...
      return repository.commit('Gengo Translations');
    }).then(function () {
      var pull;
 ...
 */
Tracker.prototype.checkExpiredOrders = function (force) {
  var self = this;
  ...
  return Order(this.bookshelf).where({status: 'pending'}).fetchAll().then(function(orders){
    ...
    orders.forEach(function(order) {
      ...
      if (force || order.get('expires_at') < Date.now()) {
        ...
        pull = self.persistAndCloseBranch(order, repository).then(function () {
...
/**
 * Recovers all completed jobs
 *
 * @param {Repository} repository
 * @param {Integer} timestamp_after
 */
Tracker.prototype.recover = function (repository, timestamp_after) {
  var self = this
    , params;

  timestamp_after = timestamp_after || 0;

  logger.gengoal('Recovering copies after ' + chalk.magenta(timestamp_after) + '...');
  ...
  this.gengo.jobs.list(params, function (err, job_list) {
    var job_ids;

    job_ids = job_list.map(function(j) {
      return j.job_id;
    });

    self.gengo.jobs.get(job_ids, function (err, jobs) {
      var languages = []
        , saves;
      ...
      jobs.jobs.forEach(function(job) {
        ...
        key_path = job.custom_data.split('.');
        locale = key_path[0];
        language = repository.find(locale);
      ...
      return Promise.all(saves)
        .then(function () {
          return repository.commit('Gengo Recovered Translations');
        });
    });
  });
};
