    ...
    fs.readdir(directory, function (err, files) {
      ...
      files.forEach(function (file) {
        ...
        try {
          ...
          logger.gengoal('Error loading file "' + name + '": ' + e);
        }
      });
...
Repository.prototype.jobs = function () {
  ...
  this.languages.forEach(function (language) {
    ...
    logger.gengoal('Checking differences between "' + language.name + '" and "' + master.name + '"...');
...
Repository.prototype.normalize = function () {
  ...
  return Promise.all(saves)
    ...
    .then(function (){
      logger.gengoal("Files normalized");
    });
};
...
Repository.prototype.decreasePending = function (branch) {
  this.completed[branch] = this.completed[branch] || 0;
